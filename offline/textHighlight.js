modelfilestrs['textHighlight'] = hereDoc(function(){/*!
	...
	var textHighlight = new function() {
		...
		this.init = function() {
			...
			if (x_currentPageXML.getAttribute("text") != "") {
				...
				if (x_currentPageXML.getAttribute('download') == 'true') {
					var txtInfo = { pageName: x_currentPageXML.getAttribute("textTitle"), pageText: x_currentPageXML.getAttribute("text") };
			...
			if (x_currentPageXML.getAttribute('download') == 'true') {
				var txtInfo = { pageName: x_currentPageXML.getAttribute("initialTitle"), pageText: x_currentPageXML.getAttribute("initialText") };
				docData.pages.push(txtInfo);
			}
			...
			if ($pageContents.data('mode') == 'Notes') {
				...
				if (x_currentPageXML.getAttribute('download') == 'true') {
					var txtInfo = { pageName: x_currentPageXML.getAttribute("notesTitle"), pageText: x_currentPageXML.getAttribute("notesText") != undefined ? x_currentPageXML.getAttribute("notesText") : '' };
					docData.pages.push(txtInfo);
				}
			}
			...
			if (x_currentPageXML.getAttribute("feedback") != undefined && x_currentPageXML.getAttribute("feedback") != "") {
				...
				if (x_currentPageXML.getAttribute('download') == 'true') {
					var txtInfo = { pageName: x_currentPageXML.getAttribute("feedbackTitle"), pageText: x_currentPageXML.getAttribute("feedback") };
					docData.pages.push(txtInfo);
				}
			}
			...
			if (x_currentPageXML.getAttribute("suggestedText") != "") {
				...
				if (x_currentPageXML.getAttribute('download') == 'true') {
					var txtInfo = { pageName: x_currentPageXML.getAttribute("suggestedTitle"), pageText: x_currentPageXML.getAttribute("suggestedText") };
					docData.pages.push(txtInfo);
				}
			}
				...
				if (key != 'lightbox') {
					...
					for (var i=0; i<info.length; i++) {
						...
						if (info[i].class == 'initTxt' || info[i].class == 'notesTxt' || info[i].class == 'suggestedTxt') {
							$holder.append('<div id="' + info[i].class + '" contenteditable="true" spellcheck="false" class="editableDiv">' + info[i].txt + '</div>');
